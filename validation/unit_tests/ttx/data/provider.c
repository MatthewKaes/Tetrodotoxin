...
#include "validation/unit_tests/ttx/data/provider_representation.h"
...
static const ttx_schema status_type = {
  1,
  1,
  TTX_SCHEMA_VALUE,
  {.value = {TTX_SCHEMA_U8, TTX_SCHEMA_LITTLE_ENDIAN}}};
static const ttx_schema word = {
  8,
  8,
  TTX_SCHEMA_VALUE,
  {.value = {TTX_SCHEMA_U64, TTX_SCHEMA_LITTLE_ENDIAN}}};
static const ttx_schema uuid =
    {16, 8, TTX_SCHEMA_RANGE, {.range = {&word, 2, 8}}};
static const ttx_schema_position args[] = {
  {&pointer, 0},
  {&uuid, 8},
  {&pointer, 24}};
static const ttx_schema input =
    {32, 8, TTX_SCHEMA_COMPOSITE, {.composite = {args, 3}}};
static const ttx_schema callable = {
  8,
  8,
  TTX_SCHEMA_MAPPING,
  {.mapping = {&input, &status_type}}};
static const ttx_schema_position query_fields[] = {
  {&pointer, offsetof(ttx_semantic_query, source)},
  {&callable, offsetof(ttx_semantic_query, bind)},
};
...
static const ttx_swizzle_mapping* selection(void) {
  return &selection_mapping;
}

static ttx_swizzle_result select_values(
...
static U32 plus_one(U32 input) {
  return input + 1;
}

static const ttx_schema primitive_function = {
  sizeof(ttx_fragment_function),
  _Alignof(ttx_fragment_function),
  TTX_SCHEMA_MAPPING,
  {.mapping = {&u32, &u32}}};
static ttx_data_status primitive_get_function(
    const void* source, Count position, ttx_fragment_function* result) {
  (void)source;
  if (position != offsetof(provider_values, function)) {
    return TTX_DATA_BOUNDS;
  }

  *result = (ttx_fragment_function)(plus_one);
  return TTX_DATA_SUCCESS;
}
...
static const ttx_schema_position primitive_fields[] = {
  ...
  {&primitive_function, offsetof(provider_values, function)},
};
...
static const ttx_fragment_access_operations primitive_access = {
  ...
  .get_function = primitive_get_function,
};
...
const provider_api* flow_provider_open(provider_compile compiler) {
  ...
  if (!four_representation) {
    ...
    selection_mapping = (ttx_swizzle_mapping){
      four_representation, prepare_representation(&two), NULL, select_position};
  }
